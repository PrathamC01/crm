...
def parse_database_error(detail: str) -> Dict[str, Any]:
    ...
    if unique_match:
        ...
        if field_match:
            ...
            error_details["constraint"] = (
                f"Duplicate value for constraint '{constraint_name}'"
            )
        return error_details

    # PostgreSQL foreign key constraint violation
    foreign_key_match = re.search(
        r'violates foreign key constraint "([^"]+)"', detail, re.IGNORECASE
    )
    if foreign_key_match:
        ...
        if key_match:
            ...
            error_details[field_name] = (
                f"Referenced record with value '{field_value}' does not exist"
            )
        else:
            error_details["reference"] = (
                f"Foreign key constraint '{constraint_name}' violation"
            )
        return error_details

    # PostgreSQL check constraint violation
    check_match = re.search(
        r'violates check constraint "([^"]+)"', detail, re.IGNORECASE
    )
    if check_match:
    ...
    enum_match = re.search(
        r'invalid input value for enum ([^:]+): "([^"]+)"', detail, re.IGNORECASE
    )
    if enum_match:
        enum_type, invalid_value = enum_match.groups()
        error_details["enum_value"] = (
            f"Invalid value '{invalid_value}' for enum type '{enum_type}'"
        )
        return error_details
...
async def custom_exception_handler(request: Request, exc: CRMBaseException):
    ...
    return JSONResponse(
        ...
        content={"status": False, "message": exc.message, "error": exc.details},
    )
...
async def http_exception_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        status_code=exc.status_code,
        content={"status": False, "message": message, "error": error},
    )
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    for err in exc.errors():
        ...
        field_path = ".".join(
            str(x)
            for x in loc
            if isinstance(x, (str, int)) and x not in ["body", "query", "path"]
        )
...
async def pydantic_validation_exception_handler(
    ...
    return JSONResponse(
        status_code=422,
        content={"status": False, "message": "Data validation failed", "error": errors},
    )
...
async def sqlalchemy_exception_handler(request: Request, exc: IntegrityError):
    ...
    return JSONResponse(
        ...
        content={"status": False, "message": message, "error": error_details},
    )
...
async def generic_exception_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(
        status_code=500,
        content={"status": False, "message": "Internal Server Error", "error": {}},
    )
