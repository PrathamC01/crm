...
async def get_companies(
    ...
    return StandardResponse(
        ...
        data=CompanyListResponse(
            companies=company_response_list, 
            total=total, 
            skip=skip, 
            limit=limit
        ),
...
async def create_company(
    ...
    if override_duplicate and user_role != "ADMIN":
        raise HTTPException(status_code=403, detail="Only Admin can override duplicate checks")
...
async def process_company_approval(
    ...
    if user_role not in ["L1_SALES_HEAD", "ADMIN"]:
...
async def upload_supporting_documents(
    ...
    # Here you would implement actual file storage (S3/MinIO)
    # For now, we'll just return success with mock file paths
    file_paths = [f"documents/{company_id}/{file.filename}" for file in files]
    
    # Update company with document paths
    company = company_service.get_company_by_id(company_id)
    if not company:
        from ...exceptions.custom_exceptions import NotFoundError
        raise NotFoundError("Company", company_id)
    ...
    updated_docs = existing_docs + file_paths
    
    # Update company (this would be done through a proper update method)
    company.supporting_documents = updated_docs
    company_service.db.commit()
    
    return StandardResponse(
        status=True,
        message="Documents uploaded successfully",
        data={"uploaded_files": file_paths}
    )
...
async def get_industry_masters():
    ...
    industry_masters = {
        "BFSI": [
            "Banking", "Insurance", "Financial Services", "Investment Banking", 
            "Asset Management", "Fintech", "Credit & Lending"
        ],
        "Government": [
            "Central Government", "State Government", "Local Bodies", "PSUs", 
            "Defense", "Public Services"
        ],
        "IT_ITeS": [
            "Software Development", "IT Services", "Product Engineering", 
            "Data Analytics", "Cloud Services", "Cybersecurity", "BPO/KPO"
        ],
        "Manufacturing": [
            "Automotive", "Textiles", "Steel & Metals", "Chemicals", 
            "Pharmaceuticals", "Electronics", "Heavy Machinery"
        ],
        "Healthcare": [
            "Hospitals", "Pharmaceuticals", "Medical Devices", "Diagnostics", 
            "Telemedicine", "Healthcare IT"
        ],
        "Education": [
            "K-12 Schools", "Higher Education", "Vocational Training", 
            "EdTech", "Online Learning", "Corporate Training"
        ],
        "Telecom": [
            "Mobile Services", "Internet Services", "Infrastructure", 
            "Satellite Communications", "Network Equipment"
        ],
        "Energy_Utilities": [
            "Power Generation", "Oil & Gas", "Renewable Energy", 
            "Utilities", "Mining", "Solar/Wind"
        ],
        "Retail_CPG": [
            "E-commerce", "Fashion & Apparel", "FMCG", "Consumer Electronics", 
            "Food & Beverage", "Grocery Retail"
        ],
        "Logistics": [
            "Transportation", "Warehousing", "Supply Chain", "Last Mile Delivery", 
            "Freight Services", "3PL Services"
        ],
        "Media_Entertainment": [
            "Broadcasting", "Digital Media", "Gaming", "Advertising", 
            "Publishing", "Entertainment Production"
        ]
    }
