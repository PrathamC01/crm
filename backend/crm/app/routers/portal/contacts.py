...
from ...schemas.contact import ContactCreate, ContactUpdate, ContactListResponse
...
async def get_contacts(
    ...
    try:
        if company_id:
            contacts = await contact_service.get_contacts_by_company(company_id, skip, limit)
        else:
            contacts = await contact_service.get_contacts(skip, limit, search)
        
        total = await contact_service.get_contact_count(search)
...
async def update_contact(
    ...
    try:
        contact = await contact_service.update_contact(contact_id, contact_data, current_user["id"])
        if not contact:
            raise HTTPException(status_code=404, detail="Contact not found")
...
async def delete_contact(
    ...
    """Soft delete contact"""
    try:
        deleted = await contact_service.delete_contact(contact_id, current_user["id"])
        if not deleted:
            raise HTTPException(status_code=404, detail="Contact not found")
...
async def get_decision_makers(
    ...
    try:
        contacts = await contact_service.get_decision_makers(company_id)
        
        return StandardResponse(
            status=True,
            message="Decision makers retrieved successfully",
            data={"decision_makers": contacts}
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))