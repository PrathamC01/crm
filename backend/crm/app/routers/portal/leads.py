...
from ...schemas.lead import (
    LeadCreate, LeadUpdate, LeadListResponse, LeadStatusUpdate, 
    LeadConversion, LeadSummary, LeadStatus
)
...
async def get_leads(
    ...
    try:
        if company_id:
            leads = await lead_service.get_leads_by_company(company_id, skip, limit)
        elif sales_person_id:
            leads = await lead_service.get_leads_by_salesperson(sales_person_id, skip, limit)
        else:
            leads = await lead_service.get_leads(skip, limit, status, search)
        
        total = await lead_service.get_lead_count(status, search)
...
async def create_lead(
    ...
    """Create new lead"""
    try:
        lead = await lead_service.create_lead(lead_data, current_user["id"])
...
async def update_lead_status(
    ...
    try:
        ...
        lead = await lead_service.update_lead(lead_id, update_data, current_user["id"])
        if not lead:
            raise HTTPException(status_code=404, detail="Lead not found")
...
async def convert_lead_to_opportunity(
    ...
    try:
        opportunity = await lead_service.convert_to_opportunity(
...
async def delete_lead(
    ...
    """Soft delete lead"""
    try:
        deleted = await lead_service.delete_lead(lead_id, current_user["id"])
        if not deleted:
            raise HTTPException(status_code=404, detail="Lead not found")
...
async def get_lead_summary(
    ...
    try:
        summary = await lead_service.get_lead_summary(sales_person_id)
...
async def auto_close_inactive_leads(
    ...
    try:
        count = await lead_service.auto_close_inactive_leads()
