...
class Opportunity(BaseModel):
    ...
    lead_id = Column(Integer, ForeignKey("leads.id"), nullable=False, unique=True, index=True)  # One-to-one mapping
    ...
    contact_id = Column(Integer, ForeignKey("contacts.id"), nullable=True, index=True)  # Primary contact
    
    # Current Stage and Status
    current_stage = Column(SQLEnum(OpportunityStage), default=OpportunityStage.L1_PROSPECT, nullable=False, index=True)
    status = Column(SQLEnum(OpportunityStatus), default=OpportunityStatus.CONVERTED_FROM_LEAD, nullable=False, index=True)
    ...
    converted_by = Column(Integer, ForeignKey("users.id"), nullable=False)  # Who converted the lead
    approval_status = Column(String(50), default="Approved")  # Since conversion requires approval
    ...
    sales_processes = relationship("SalesProcess", back_populates="opportunity", cascade="all, delete-orphan")
    ...
    def __init__(self, **kwargs):
        ...
        if 'lead_id' not in kwargs or kwargs['lead_id'] is None:
            raise ValueError("Opportunities can only be created by converting a Lead. lead_id is required.")
    ...
    def current_sales_process(self):
        ...
        if self.sales_processes:
            return next((sp for sp in self.sales_processes if sp.stage.value == self.current_stage.value), None)
        return None

    @property
    def can_create_quotations(self):
        """Check if quotations can be created (L3 stage completed)"""
        return self.l3_completed or self.current_stage in [OpportunityStage.L3_PROPOSAL, OpportunityStage.WIN]
    ...
    def create_from_lead(cls, lead, converted_by_user_id, approved_by_user_id=None):
        ...
        opportunity_data = {
            ...
            'lead_data': {
                ...
                'lead_sub_type': lead.lead_sub_type.value if lead.lead_sub_type else None,
                'tender_sub_type': lead.tender_sub_type.value if lead.tender_sub_type else None,
