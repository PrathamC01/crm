...
class Lead(BaseModel):
    ...
    convert_to_opportunity_date = Column(Date, nullable=False)
    ...
    sales_person = relationship(
        "User", foreign_keys=[sales_person_id], back_populates="leads_assigned"
    )
    conversion_requester = relationship("User", foreign_keys=[conversion_requested_by])
    ...
    def creator_name(self):
        return (
            ...
            if self.creator and hasattr(self.creator, "full_name")
            else (self.creator.name if self.creator else None)
        )

    @property
    def sales_person_name(self):
        return (
            self.sales_person.full_name
            if self.sales_person and hasattr(self.sales_person, "full_name")
            else (self.sales_person.name if self.sales_person else None)
        )

    @property
    def conversion_requester_name(self):
        return (
            self.conversion_requester.full_name
            if self.conversion_requester
            and hasattr(self.conversion_requester, "full_name")
            else (self.conversion_requester.name if self.conversion_requester else None)
        )
    ...
    def reviewer_name(self):
        return (
            ...
            if self.reviewer and hasattr(self.reviewer, "full_name")
            else (self.reviewer.name if self.reviewer else None)
        )
    ...
    def can_request_conversion(self):
        ...
        return (
            ...
            and not self.conversion_requested
        )
