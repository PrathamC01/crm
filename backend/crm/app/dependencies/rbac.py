...
async def require_users_read(current_user: dict = Depends(get_current_user)) -> dict:
    return await require_permission("users:read", current_user)
...
async def require_companies_read(current_user: dict = Depends(get_current_user)) -> dict:
    return await require_any_permission(["companies:read", "companies:all"], current_user)

async def require_companies_write(current_user: dict = Depends(get_current_user)) -> dict:
    return await require_any_permission(["companies:write", "companies:all"], current_user)
...
async def require_contacts_write(current_user: dict = Depends(get_current_user)) -> dict:
    return await require_any_permission(["contacts:write", "contacts:all"], current_user)
...
async def require_opportunities_read(current_user: dict = Depends(get_current_user)) -> dict:
    return await require_any_permission(["opportunities:read", "opportunities:all"], current_user)

async def require_opportunities_write(current_user: dict = Depends(get_current_user)) -> dict:
    return await require_any_permission(["opportunities:write", "opportunities:all"], current_user)
...
async def require_marketing_or_sales_role(current_user: dict = Depends(get_current_user)) -> dict:
    """Require marketing, sales, or admin role"""
    role_name = current_user.get('role_name', '')
    if role_name not in ['marketing', 'sales_manager', 'sales_executive', 'admin', 'super_admin']:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
