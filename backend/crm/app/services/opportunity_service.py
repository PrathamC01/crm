...
from ..models import (
    Opportunity, Lead, Company, Contact, User, RoleType, 
    OpportunityStatus, OpportunityStage, QualificationStatus,
    GoNoGoStatus, QuotationStatus
)
...
class OpportunityService:
    ...
    def create_opportunity(self, opportunity_data: dict, created_by: Optional[int] = None) -> Opportunity:
        ...
        db_opportunity = Opportunity(
            ...
            requirement_gathering_notes=opportunity_data.get('requirement_gathering_notes'),
            go_no_go_status=opportunity_data.get('go_no_go_status', GoNoGoStatus.PENDING),
            qualification_completed_by=opportunity_data.get('qualification_completed_by'),
            ...
            qualification_meeting_completed=opportunity_data.get('qualification_meeting_completed', False),
            qualification_meeting_date=opportunity_data.get('qualification_meeting_date'),
            qualification_meeting_notes=opportunity_data.get('qualification_meeting_notes'),
            
            # L3 - Proposal / Bid Submission Fields
            quotation_created=opportunity_data.get('quotation_created', False),
            quotation_status=opportunity_data.get('quotation_status', QuotationStatus.DRAFT),
            ...
            solutions_team_approval_notes=opportunity_data.get('solutions_team_approval_notes'),
            ...
            updated_proposal_file_path=opportunity_data.get('updated_proposal_file_path'),
            updated_proposal_submitted=opportunity_data.get('updated_proposal_submitted', False),
            negotiated_quotation_file_path=opportunity_data.get('negotiated_quotation_file_path'),
            negotiation_rounds=opportunity_data.get('negotiation_rounds', 0),
            commercial_approval_required=opportunity_data.get('commercial_approval_required', False),
            commercial_approval_status=opportunity_data.get('commercial_approval_status'),
            
            # L5 - Won Fields
            kickoff_meeting_scheduled=opportunity_data.get('kickoff_meeting_scheduled', False),
    ...
    def _generate_unique_pot_id(self) -> str:
        ...
        while True:
            ...
            existing = self.db.query(Opportunity).filter(Opportunity.pot_id == pot_id).first()
            if not existing:
                return pot_id
    ...
    def update_opportunity(self, opportunity_id: int, opportunity_data: dict, updated_by: Optional[int] = None) -> Optional[Opportunity]:
        ...
        for field, value in opportunity_data.items():
            if field not in ['id', 'pot_id', 'created_on', 'created_by'] and value is not None:
    ...
    def update_stage(self, opportunity_id: int, stage: str, updated_by: int, notes: str = None, stage_specific_data: Dict[str, Any] = None) -> Optional[Opportunity]:
        ...
        db_opportunity.probability = stage_probabilities.get(OpportunityStage(stage), db_opportunity.probability)
    ...
    def update_qualification(self, opportunity_id: int, qualification_data: Dict[str, Any], updated_by: int) -> Optional[Opportunity]:
        ...
        for field in ['requirement_gathering_notes', 'go_no_go_status', 'qualification_status', 
                     'qualification_scorecard', 'qualification_completed_by']:
    ...
    def update_demo_tasks(self, opportunity_id: int, demo_data: Dict[str, Any], updated_by: int) -> Optional[Opportunity]:
        ...
        # Update demo fields
        for field in ['demo_completed', 'demo_date', 'demo_summary', 'presentation_materials',
                     'qualification_meeting_completed', 'qualification_meeting_date', 'qualification_meeting_notes']:
    ...
    def update_proposal_tasks(self, opportunity_id: int, proposal_data: Dict[str, Any], updated_by: int) -> Optional[Opportunity]:
        ...
        for field in ['quotation_created', 'quotation_status', 'quotation_file_path', 'quotation_version',
                     'proposal_prepared', 'proposal_file_path', 'proposal_submitted', 'proposal_submission_date',
                     'poc_completed', 'poc_notes', 'solutions_team_approval_notes']:
    ...
    def update_negotiation_tasks(self, opportunity_id: int, negotiation_data: Dict[str, Any], updated_by: int) -> Optional[Opportunity]:
        ...
        for field in ['customer_discussion_notes', 'proposal_updated', 'updated_proposal_file_path',
                     'updated_proposal_submitted', 'negotiated_quotation_file_path', 'negotiation_rounds',
                     'commercial_approval_required', 'commercial_approval_status']:
    ...
    def update_won_tasks(self, opportunity_id: int, won_data: Dict[str, Any], updated_by: int) -> Optional[Opportunity]:
        ...
        # Update won fields
        for field in ['kickoff_meeting_scheduled', 'kickoff_meeting_date', 'loi_received', 'loi_file_path',
                     'order_verified', 'handoff_to_delivery', 'delivery_team_assigned']:
        ...
        if (won_data.get('handoff_to_delivery') and 
            db_opportunity.loi_received and 
            db_opportunity.order_verified):
    ...
    def close_opportunity(self, opportunity_id: int, status: str, close_date: str, updated_by: int, notes: str = None, lost_reason: str = None, competitor_name: str = None, drop_reason: str = None) -> Optional[Opportunity]:
    ...
    def get_pipeline_summary(self, user_id: int = None) -> dict:
        ...
        closing_stage_count = len([opp for opp in opportunities if opp.stage in [OpportunityStage.L4_NEGOTIATION, OpportunityStage.L5_WON]])
