...
class LeadService:
    ...
    def get_lead_by_id(self, lead_id: int) -> Optional[Lead]:
        ...
        return (
            self.db.query(Lead)
            .options(joinedload(Lead.company), joinedload(Lead.sales_person))
            .filter(
                and_(
                    Lead.id == lead_id,
                    Lead.is_active == True,
    ...
    def get_leads(
        self, skip: int = 0, limit: int = 100, status: str = None, search: str = None
    ) -> List[Lead]:
        ...
        return (
            query.order_by(Lead.last_activity_date.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_leads_by_salesperson(
        self, sales_person_id: int, skip: int = 0, limit: int = 100
    ) -> List[Lead]:
    ...
    def convert_to_opportunity(
        self, lead_id: int, conversion_data: dict, created_by: int
    ) -> dict:
        """Convert lead to opportunity"""
        conversion_dict = conversion_data.dict()  # Pydantic v1
        ...
        contact = (
            ...
            .filter(Contact.id == conversion_dict["contact_id"])
            .first()
        )
        ...
        # Create opportunity
        # or: conversion_data.model_dump()        # Pydantic v2

        db_opportunity = Opportunity(
            lead_id=lead_id,
            company_id=db_lead.company_id,
            contact_id=conversion_dict["contact_id"],
            name=conversion_dict["opportunity_name"],
            stage=conversion_dict.get("stage", "L1"),
            amount=conversion_dict.get("amount"),
            justification=conversion_dict.get("justification", "Test Justification"),
            notes=conversion_dict.get("notes"),
            created_by=created_by,
        )
