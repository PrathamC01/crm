...
const OpportunitiesPage = () => {
  ...
  const opportunityColumns = [
    ...
    },
    {
      ...
      render: (opportunity) => {
        ...
        return (
          <span className={`px-2 py-1 text-xs font-semibold rounded-full ${statusColors[opportunity.status] || 'bg-gray-100 text-gray-800'}`}>
            {opportunity.status}
          </span>
        );
      }
    },
    {
      ...
      render: (opportunity) => (
        <div className="text-right">
          <div className="font-medium">₹{Number(opportunity.amount || 0).toLocaleString()}</div>
          <div className="text-sm text-gray-500">{opportunity.currency || 'INR'}</div>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      label: 'Owner',
      render: (opportunity) => (
        <div className="text-sm text-gray-600">{opportunity.converted_by_name}</div>
      )
    }
  ];
  ...
  const fetchStats = async () => {
    try {
      ...
      setStats({
        ...
        winRate: opportunities.length > 0 ? Math.round((opportunities.filter(o => o.status === 'Won').length / opportunities.length) * 100) : 0
      });
    } catch (error) {
      console.error('Error fetching opportunity stats:', error);
      setStats({ total: 0, open: 0, won: 0, lost: 0, totalValue: 0, winRate: 0 });
    }
  };
  ...
  const actions = [
    ...
    },
    {
      ...
      show: (opportunity) => opportunity.current_stage === 'L3_Proposal' && opportunity.status === 'Open'
    }
  ];
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            <div className="flex-1">
              ...
              <p className="text-2xl font-bold text-yellow-600">{stats.winRate}%</p>
            </div>
        ...
        {selectedOpportunity && (
          <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <h3 className="font-semibold text-gray-700">POT ID</h3>
                <p className="text-blue-600 font-medium">{selectedOpportunity.pot_id}</p>
              </div>
              <div>
                <h3 className="font-semibold text-gray-700">Opportunity Name</h3>
                <p className="text-gray-900">{selectedOpportunity.name}</p>
              </div>
              <div>
                <h3 className="font-semibold text-gray-700">Company</h3>
                <p className="text-gray-900">{selectedOpportunity.company_name}</p>
              </div>
              <div>
                <h3 className="font-semibold text-gray-700">Current Stage</h3>
                <p className="text-gray-900">{selectedOpportunity.current_stage}</p>
              </div>
              <div>
                <h3 className="font-semibold text-gray-700">Value</h3>
                <p className="text-gray-900">₹{Number(selectedOpportunity.amount || 0).toLocaleString()}</p>
              </div>
              <div>
                <h3 className="font-semibold text-gray-700">Probability</h3>
            ...
            <div>
              <h3 className="font-semibold text-gray-700 mb-4">Sales Process Progress</h3>
              <div className="space-y-2">
                {['L1_Prospect', 'L2_Need_Analysis', 'L3_Proposal', 'Win'].map((stage, index) => {
                  const isCompleted = selectedOpportunity[`${stage.toLowerCase()}_completed`];
                  const isCurrent = selectedOpportunity.current_stage === stage;
                  
                  return (
                    <div key={stage} className="flex items-center">
                      <div className={`w-4 h-4 rounded-full mr-3 ${isCompleted ? 'bg-green-500' : isCurrent ? 'bg-blue-500' : 'bg-gray-300'}`}></div>
                      <span className={`${isCurrent ? 'font-semibold text-blue-600' : 'text-gray-700'}`}>
                        {stage.replace('_', ' ')}
                      </span>
                      {isCompleted && (
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t">
              ...
              {selectedOpportunity.status === 'Open' && (
                <button
                  onClick={() => setShowSalesProcessModal(true)}
