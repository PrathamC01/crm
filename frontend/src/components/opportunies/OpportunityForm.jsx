...
// Helper: Format datetime for inputs
const formatDateTime = (date) =>
  date ? new Date(date).toISOString().slice(0, 16) : null;

const OpportunityForm = ({ opportunity, opportunity_id, onSave, onCancel }) => {
  ...
  // Fetch opportunity and prefill form
  const fetchOpportunity = async () => {
    try {
      const response = await apiMethods.opportunities.getOpportunity(
        opportunity_id
      );
      const opp = response?.data?.data?.opportunity;
      if (opp) {
        setFormData((prev) => ({
          ...prev,
          ...opp,
          demo_date: formatDateTime(opp.demo_date),
          qualification_meeting_date: formatDateTime(
            opp.qualification_meeting_date
          ),
          proposal_submission_date: formatDateTime(
            opp.proposal_submission_date
          ),
          kickoff_meeting_date: formatDateTime(opp.kickoff_meeting_date),
          close_date: opp.close_date || null,
          follow_up_date: opp.follow_up_date || null,
          reactivate_date: opp.reactivate_date || null,
  ...
  const fetchInitialData = async () => {
    try {
      const [leadsRes, companiesRes, usersRes] = await Promise.all([
        api.get("/api/leads?status=Qualified"),
        api.get("/api/companies"),
        api.get("/api/users"),
      ]);
      if (leadsRes.status) setLeads(leadsRes.data.data.leads || []);
      if (companiesRes.status)
        setCompanies(companiesRes.data.data.companies || []);
      if (usersRes.status) setUsers(usersRes.data.data.users || []);
      console.log({
        leadsRes: leadsRes.data.data.leads,
        companiesRes: companiesRes.data.data.companies,
        usersRes: usersRes.data.data.users,
      });
    } catch (err) {
  ...
  // Fetch decision makers whenever company_id changes
  useEffect(() => {
  ...
  const validateForm = () => {
    ...
    if (isEmpty(formData.name)) newErrors.name = "Opportunity name is required";
    if (isEmpty(formData.company_id))
      newErrors.company_id = "Company is required";
    if (isEmpty(formData.contact_id))
    ...
    if (formData.amount) {
      ...
      } else if (amount >= 1000000 && isEmpty(formData.justification)) {
