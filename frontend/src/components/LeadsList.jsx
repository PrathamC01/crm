  ...
  return (
    <div className="space-y-6">
                    ...
                    <div className="flex-shrink-0">
                      <div className="h-10 w-10 rounded-full bg-blue-100 flex items-center justify-center">
                        <span className="text-sm font-medium text-blue-600">
                          {lead.project_title ? lead.project_title.charAt(0).toUpperCase() : 'L'}
                        </span>
                      </div>
                    </div>
                    <div className="ml-4">
                      ...
                      <div className="text-sm text-gray-500">
                        ...
                        {lead.expected_revenue && (
                          <span> • Expected Revenue: ₹{parseFloat(lead.expected_revenue).toLocaleString()}</span>
                        )}
                      </div>
                    </div>
                  </div>
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        <h4 className="text-sm font-medium text-blue-900 mb-3">Lead Status Workflow</h4>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 text-xs text-blue-800">
          <div>
            <StatusBadge status="New" /> → <StatusBadge status="Active" /> → <StatusBadge status="Contacted" />
          </div>
          <div>
            <StatusBadge status="Contacted" /> → <StatusBadge status="Qualified" /> → <StatusBadge status="Converted" />
          </div>
          <div>
            Or → <StatusBadge status="Unqualified" /> / <StatusBadge status="Rejected" />
          </div>
        </div>
      </div>
    </div>
