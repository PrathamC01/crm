...
const ConversionWorkflow = ({ lead, currentUser, onUpdate, onClose }) => {
  ...
  const canConvert = isAdmin || (lead.review_status === 'Approved' && lead.reviewed);
  ...
  const handleRequestConversion = async () => {
    ...
    try {
      const response = await apiRequest(`/api/leads/${lead.id}/request-conversion`, {
        method: 'POST',
        body: JSON.stringify({ notes })
      });

      if (response.status) {
        alert('Conversion request submitted successfully. Waiting for admin review.');
        onUpdate();
      } else {
  ...
  const handleApproveReject = async (decision) => {
    ...
    const comments = prompt(`Enter comments to ${actionText} this conversion request:`);
    ...
    try {
      const response = await apiRequest(`/api/leads/${lead.id}/review`, {
        method: 'POST',
        body: JSON.stringify({ 
          decision: decision, 
      ...
      if (response.status) {
        ...
        setError(response.message || `Failed to ${actionText} conversion request`);
      }
    } catch (err) {
      setError('Network error occurred');
    } finally {
  ...
  const handleConvertToOpportunity = async () => {
    ...
    if (!canConvert) {
      alert('❗ This opportunity needs to be reviewed by an Admin before it can be converted.');
      return;
    }

    const opportunityName = prompt('Enter opportunity name:', `${lead.project_title} Opportunity`);
    ...
    try {
      const response = await apiRequest(`/api/leads/${lead.id}/convert-to-opportunity`, {
        method: 'POST',
        body: JSON.stringify({ 
      ...
      if (response.status) {
        alert(`Lead successfully converted to opportunity: ${response.data.opportunity_pot_id}`);
        onUpdate();
        onClose();
      } else {
    ...
    <div className="bg-white rounded-lg shadow-lg max-w-2xl mx-auto">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="bg-gray-50 p-4 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              ...
              <span className={`ml-2 px-2 py-1 rounded-full text-xs font-semibold ${
                lead.review_status === 'Approved' ? 'bg-green-100 text-green-800' :
                lead.review_status === 'Rejected' ? 'bg-red-100 text-red-800' :
            ...
            <div>
              ...
              <span className={`ml-2 ${lead.conversion_requested ? 'text-green-600' : 'text-gray-600'}`}>
                {lead.conversion_requested ? '✓ Yes' : '✗ No'}
              </span>
            </div>
            <div>
              <span className="text-gray-500">Already Converted:</span>
              <span className={`ml-2 ${lead.converted ? 'text-green-600' : 'text-gray-600'}`}>
        ...
        <div className="bg-blue-50 p-4 rounded-lg">
          <h4 className="font-medium text-blue-900 mb-3">Conversion Workflow</h4>
          <div className="flex items-center space-x-4">
            {/* Step 1: Qualification */}
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                lead.status === 'Qualified' ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-600'
              }`}>
                1
              </div>
            ...
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                lead.conversion_requested ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-600'
              }`}>
                2
              </div>
            ...
            <div className={`flex-1 h-1 ${
              lead.reviewed && lead.review_status === 'Approved' ? 'bg-green-500' : 'bg-gray-300'
            }`}></div>

            {/* Step 3: Admin Review */}
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                lead.reviewed && lead.review_status === 'Approved' ? 'bg-green-500 text-white' : 
                lead.reviewed && lead.review_status === 'Rejected' ? 'bg-red-500 text-white' :
            ...
            <div className="flex items-center">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${
                lead.converted ? 'bg-green-500 text-white' : 'bg-gray-300 text-gray-600'
              }`}>
                4
              </div>
            ...
            <div className="space-y-2 text-sm">
              {lead.conversion_request_date && (
                <div>
                  ...
                  <span className="ml-2">{new Date(lead.conversion_request_date).toLocaleString()}</span>
                </div>
              )}
              ...
              {lead.review_date && (
                <div>
                  ...
                  <span className="ml-2">{new Date(lead.review_date).toLocaleString()}</span>
                </div>
              )}
        ...
        {lead.converted && (
          <div className="bg-green-50 p-4 rounded-lg">
            <h4 className="font-medium text-green-900 mb-3">Conversion Complete</h4>
            <div className="space-y-2 text-sm">
              {lead.conversion_date && (
                <div>
                  <span className="text-gray-700">Converted On:</span>
                  <span className="ml-2">{new Date(lead.conversion_date).toLocaleString()}</span>
                </div>
              )}
              {lead.converted_to_opportunity_id && (
                <div>
                  <span className="text-gray-700">Opportunity ID:</span>
                  <span className="ml-2">#{lead.converted_to_opportunity_id}</span>
                </div>
              )}
        ...
        {!lead.converted && (
          <div className="flex justify-between items-center pt-4 border-t">
            <div className="text-sm text-gray-500">
              {isAdmin ? (
                <span className="text-green-600">✓ You have admin privileges</span>
              ) : (
            ...
            <div className="flex space-x-3">
              ...
              {lead.can_convert_to_opportunity || (isAdmin && lead.status === 'Qualified') ? (
                <button
                ...
                <button
                  onClick={() => alert('❗ This opportunity needs to be reviewed by an Admin before it can be converted.')}
