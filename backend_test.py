...
class CRMBackendTester:
    ...
    def test_masters_endpoints(self):
        ...
        # Test get UOMs
        try:
            response = self.make_request("GET", "/api/masters/uoms", headers=self.auth_headers)
            if response.status_code == 200:
                data = response.json()
                if data.get("status") and "data" in data:
        ...
        try:
            response = self.make_request("GET", "/api/masters/departments", headers=self.auth_headers)
            if response.status_code == 200:
                data = response.json()
                if data.get("status") and "data" in data:
